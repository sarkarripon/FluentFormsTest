...
class PreCheckCest
{
    ...
    public function formcr(AcceptanceTester $I, Example $example)
    {
        ...
        $I->wait(1);
        $remoteData = json_decode($this->fetchPlatformlyData($example['email']));
        $I->wait(1);
        ...
        if (!empty($absentData)) {
            $I->fail() . " is not present in the remote data";
        }else{
            echo 'Integration test pass! All data is present in the remote';
        }
